...
Si $E_{1}, E_{2},... \in \mathds{B}$ es una sucesión de conjuntos disjuntos, entonces:
\begin{equation*}
...
\begin{defi}
Sean dos sucesos, $E, E' \in \mathds{B}$, diremos que son independientes si $P(E\cap E') = P(E) P(E')$.
\end{defi}
...
\begin{defi}
Sean $A$, $A'$ dos variables aleatorias diremos que son independientes si todos los sucesos de sus $\sigma$-álgebras $\mathds{B}$ y $\mathds{B'}$,respectivamente, lo son.
\end{defi}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TODO:
% Hablar de las mismas cosas de las VA discretas pero para las continuas 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$$
\sigma^2(A) \equiv \mathds{E}_c(A^2) = \sum_{x_m\in E} P(A=x_m) \big(x_m - \mu\big)^2 = \mathds{E}(A^2) - \mu^2.
$$
\end{defi}

\begin{defi}
A la raíz cuadrada positiva de la varianza la notaremos $\sigma(A)=+\sqrt{\sigma^2(A)}$ y diremos que es la desviación estándar de la variable aleatoria discreta A. 
\end{defi}
...
\begin{teorema}
Sean $A_{1},A_{2},...,A_{n}$ variables aleatorias independientes, con esperanza $\mu=0$, varianza $\sigma^2$ e idénticamente distribuidas, es decir, obtenidas de la misma distribución. Entonces $$
\frac{ \frac{1}{n}\sum_{i=1}^nA_i - \mu}{ \frac{\sigma}{\sqrt{n}}} \to^d N(0,1).
$$
...
$$
\abs{ exp(iz) } = \abs{\cos z+i\sin z} = \sqrt{( \cos^2 z - \sin^2 z)} \leq 1.
$$
...
\item Sean $c,b\in \mathds{R}$ entonces $\phi_{cA+b}(t)=e^{itb}\phi_A(ct)$.
...
$$
\item Si $\mathds{E}(A^k) < \infty$ su derivada k-ésima evaluada en 0 es $\phi_A^{(k)}(0)=i^k\mathds{E}(A^k), \quad k=1,..,n$.
\end{itemize}
...
Sea ${ A_n }_{n \in \mathds{N}}$ una sucesión de variables aleatorias tal que la sucesión de sus funciones características, ${\phi_{A_n}}_{n \in \mathds{N}}$ converge a $\phi_{A}(t)$ para cada $t\in \mathds{R}$, donde $\phi_{A}$ es continua en 0. Entonces existe una variable aleatoria $A$ con función característica $\phi$ y $A_n \to^d A$.
\end{teorema}

\begin{proof}[Demostración (Teorema central del límite)]

%% Empieza la demostración de verdad
Sean $A_{1},A_{2},...,A_{n}$ variables aleatorias independientes identicamente distribuidas con esperanza $\mu < \infty$ y varianza $\sigma^2< \infty$.
Sea ahora $$
Z_{n}=\frac{\overline{A}-\mu }{\frac{\sigma }{\sqrt{n}}} = \frac{A_{1}+A_{2}+...+A_{n}-n\mu }{\sigma \sqrt{n}}.
$$
...
Así obtenemos que $$
\big(\phi_{\frac{Y_1}{\sqrt{n}}}(t) \big)^n =\big( 1 + 0 - \frac{t^2}{2n} + \frac{t^2}{n} h_2(\frac{t}{\sqrt{n}}) \big)^n\longrightarrow e^{-\frac{t^2}{2}}.
$$

cuyo límite es la función característica de una variable perteneciente a una distribución normal con media 0 y varianza 1, concluimos la demostración aplicando el Teorema de Levy.
\end{proof}
